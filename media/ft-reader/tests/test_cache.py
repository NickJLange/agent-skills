...
from ft_reader.cache import Cache
...
def test_ttl_expiry(tmp_cache_dir, monkeypatch):
    c = Cache(tmp_cache_dir)
    c.set_json("GET", "https://example.test/b", {"y": 2})
    # Rewind fetched_at to simulate an expired entry.
    import json
...
def test_bytes_roundtrip(tmp_cache_dir):
    c = Cache(tmp_cache_dir)
    path = c.set_bytes("GET", "https://cdn.test/x.mp3", b"\x00\x01\x02")
    assert path.read_bytes() == b"\x00\x01\x02"
    assert c.get_bytes_path("GET", "https://cdn.test/x.mp3", ttl=1000) == path
